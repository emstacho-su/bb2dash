// bb2dash :: edge function `calendar-push`  (v1)
...
import { createGoogleCalendar, exchangeRefreshToken, type PushItem } from "./google.ts";
...
/**
 * Close the run and the settings row together. gcal_dirty is cleared ONLY on a clean run, so a
 * partial or failed push is retried by the next tick without anyone scheduling a retry.
 */
async function finish(
  ...
  await supabase.from("calendar_push_runs")
    .update({ status, counts, finished_at: finishedAt, error: error?.slice(0, 2000) ?? null })
    .eq("id", runId);

  const settings: Record<string, unknown> = {
    gcal_push_request_id: null,
    gcal_last_push_at: finishedAt,
    gcal_last_status: status,
    gcal_last_error: error?.slice(0, 2000) ?? null,
  };
  if (status === "ok") settings.gcal_dirty = false;
  await supabase.from("app_settings").update(settings).eq("id", true);
}
...
Deno.serve(async (req: Request) => {
  ...
  // 2. Which run row is this? The tick opens one and passes its id; a hand invocation opens one.
  let runId: number;
  try {
    ...
    runId = typeof given === "number" && Number.isFinite(given) ? given : await openRun();
  } catch (cause) {
  ...
  try {
    ...
    const { data: settings, error: settingsError } = await supabase
      .from("app_settings").select("gcal_enabled, gcal_calendar_id").eq("id", true).single();
    if (settingsError) throw new Error(`app_settings: ${settingsError.message}`);

    if (!settings?.gcal_enabled) {
      await finish(runId, "failed", {}, "gcal_enabled is false; nothing was pushed");
      return json({ run_id: runId, status: "failed", error: "gcal_enabled is false" }, 200);
    }
    ...
    if (!calendarId || calendarId === "primary") {
      ...
      return json({ run_id: runId, status: "failed", error: message }, 200);
    }
    ...
    if (!token.accessToken) {
      const message = token.invalidGrant
    ...
    try {
      await finish(runId, "failed", {}, message);
    } catch { /* the run row is best-effort once we are already failing */ }
