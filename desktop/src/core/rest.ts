...
import type { RestGet, WebSession } from './types';
...
export interface RestOptions {
  ...
  /**
   * Share one throttle across instances. A per-tick reader (see
   * `createRestGetForToken`) is a new object every tick, and a throttle of its own
   * would log every failure of every tick — C-6 says once per relation per hour.
   */
  readonly shouldLog?: (relation: string, nowMs: number) => boolean;
}
...
export function createRestGet(options: RestOptions): RestGet {
  ...
  const shouldLog = options.shouldLog ?? createErrorThrottle();
...
/**
 * A reader bound to one already-read session (C-5).
 *
 * The poller reads the partition's cookie once at the top of a tick and then makes
 * three relation reads; `createRestGet`'s per-call `getAccessToken` would re-open
 * the cookie jar for each of them. This variant is what `PollerDeps.createRest`
 * wants, and it is the seam a container uses when the token arrives from the
 * environment instead of a cookie.
 *
 * `shouldLog` should be a throttle shared with every other instance, or C-6's
 * "once per relation per hour" becomes once per relation per tick.
 */
export function createRestGetForToken(
  options: Omit<RestOptions, 'getAccessToken'> & { readonly session: WebSession },
): RestGet {
  const { session, ...transport } = options;
  return createRestGet({ ...transport, getAccessToken: () => Promise.resolve(session.accessToken) });
}
