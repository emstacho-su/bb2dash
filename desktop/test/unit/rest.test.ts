...
describe('createRestGetForToken', () => {
  const session = { accessToken: TOKEN, expiresAt: 4102444800 };

  it('sends the bound token and never asks for another', async () => {
    ...
    const get = createRestGetForToken({
      supabaseUrl: SUPABASE_URL,
      anonKey: ANON_KEY,
      session,
      fetchImpl,
    ...
    await get('v_sync_status', 'select=id', asRows);
    await get('v_work_items', 'select=id', asRows);
  ...
  it('shares an injected throttle, so per-tick readers do not log every failure', async () => {
    const logged: string[] = [];
    const shouldLog = createErrorThrottle(60_000);
    const fetchImpl: FetchLike = () => Promise.resolve(jsonResponse({ message: 'no' }, 500));

    // A new reader per tick is exactly how `PollerDeps.createRest` is used.
    const readerForTick = (): ReturnType<typeof createRestGetForToken> =>
      createRestGetForToken({
        supabaseUrl: SUPABASE_URL,
        anonKey: ANON_KEY,
        session,
    ...
    for (let tick = 0; tick < 3; tick += 1) {
      await expect(readerForTick()('v_sync_status', '', asRows)).rejects.toThrow(RestError);
    }

    expect(logged).toHaveLength(1);
  });

  it('without a shared throttle every new reader logs again (the bug this guards)', async () => {
    const logged: string[] = [];
    const fetchImpl: FetchLike = () => Promise.resolve(jsonResponse({ message: 'no' }, 500));
    ...
    for (let tick = 0; tick < 3; tick += 1) {
      ...
      await expect(get('v_sync_status', '', asRows)).rejects.toThrow(RestError);
    }
