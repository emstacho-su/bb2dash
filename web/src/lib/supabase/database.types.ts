// Generated by `supabase gen types`, with ONE hand edit: `attention_items`
// gained `archived_at`, `archived_by` and `decision` in migration 090 and this
// file predates it. `state` needed no widening — it is generated as `string`,
// not a union, so the fourth value `'archived'` already fits. The next
// regeneration supersedes both notes and this comment.
export type Json =
  | string
  | number
  | boolean
