 ...
 * their aggregation, parents add up their children in capacity units, and extra
 * credit adds to earned but never to capacity.
 *
 * Phase 12b (G-1): muting is gone, so every node counts. `r` stays because it
 * is how the per-aggregation table expresses the arithmetic — production only
 * ever asks for `r = null`, graded so far.
 *
 * Contract: `68_PHASE10B_grade_model.md` §Engine, Semantics "Capacity" and the
