...
const MODEL: GradesModelProps = {
  standings: {
    'IST.323': { result: NOT_COMPUTED_MANUAL, error: null, loading: false },
    'IST.466': { result: makeComputed({ usesHypotheticals: true }), error: null, loading: false },
  },
...
describe('GradesScreen — the read-only model line', () => {
  ...
  it('says loading for a course whose model has not been computed yet, and a failure as an alert', () => {
    render(
      <GradesScreen
        model={{
          standings: { 'IST.323': { result: null, error: 'Could not load the grade model: timeout', loading: false } },
          history: new Map(),
