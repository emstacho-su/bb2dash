...
- `db/migrations/012_hybrid_similarity.sql` — hybrid RRF gains a `p_min_similarity` floor (vector arm only) and a real `similarity` column; callers default to 0.78. `supabase/functions/search` v3 forwards it; `mcp-server/` is the Claude Code retrieval client (see its README).
