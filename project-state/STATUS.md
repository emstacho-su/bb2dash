...
> migrations 060–065 live; the live calendar proof waits on Stack's one-time Google consent).
...
| Typed warehouse | migrations 001–045 on `main`, 046–051 (Phase 10a) and 060–065 (Phase 11) live from the in-flight branches (repo numbering; see note below); 7 courses, 80 assignments, 145 sessions, planner tables; `attention_items`, `agent_requests`, `app_settings` (+ `gcal_*`), `calendar_events` mirror, `calendar_push_runs`, `v_calendar_push_items`, `v_announcements_unread`; every public view `security_invoker`, anon revoked |
...
| Edge functions | `embed-corpus` **v5** (resume-safe batch embedder; chunks by code point), `search` **v5** (retrieval API; **default mode: hybrid**; optional `min_similarity` floor; optional `include_superseded`), `calendar-push` **v1** (Google Calendar upsert/delete by deterministic event id; `verify_jwt` off, `x-push-secret` from Vault; fired by pg_cron `bb2dash-calendar-push` when `app_settings.gcal_dirty`) |
...
10. **Phase 11 — Planner, Google Calendar push, bell** (`feat/planner-11`, PR open, 2026-09-15; brief
   ...
   worktree because the main checkout was Phase 10a's. Migrations **060–065**: `calendar_events`
   ...
   Contract had said the fold's `started_at`, which flagged 22 of 64 items absent). Edge function
   `calendar-push` v1 (fetch client, no SDK; insert / patch / delete diff against the mirror; zero
   ...
   published, rows stay tentative. Tests: web 450 (from 345), function 25 (`node --test`).
...
| 11 | Planner + Google Calendar push, announcements bell/page, data gaps | `69_PHASE11_planner.md` | **PR open** (`feat/planner-11`, migrations 060–065 live); live calendar proof after Stack's consent step |
| 12 | Electron shell | — | |
...
Phase 11 = 060–069 (060–065 used), Phase 12 = 070–079 if needed. Both phase branches cut from `main`
