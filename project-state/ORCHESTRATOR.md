...
| 11 | Planner week grid, Google Calendar push, bell + Announcements page, data gaps | **PR open**; live calendar proof after Stack's consent step | — | 060–069 (060–065 live) |
...
* Phase 11: finish the Google setup (Cloud project under the Gmail account, OAuth Desktop client,
  then `node scripts/google-consent.mjs` choosing `emstacho@g.syr.edu`); walk the six-step
  acceptance script on the preview; say when to merge. The PM flips `gcal_enabled` and records the
  three-run proof once the consent script has run.
