...
**Sprint 2 — planning** (rewritten 2026-09-22 after the close: Stack wants requirements translated, not prompts; four stages, each with a stop)

> /bb2dash-pm Plan sprint 2. Sprint 1 closed on 2026-09-22 (PR #25); read `docs/planning/README.md` for where the planning docs live now, then `docs/planning/sprint-2/90_SPRINT2_INTAKE.md`. Work in a worktree `C:/Users/estac/projects/bb2dash-wt-sprint2-plan` on branch `docs/sprint2-planning` cut from `origin/main`; never commit in the shared checkout; push after every stage; migrations for sprint 2 start at 091; file numbers continue the repo-wide sequence (91, 92, … and past 99 if needed). This session runs four stages and STOPS at the end of each one for me.
>
> **Stage A — translate what sprint 1 left undone into requirements (not the old prompts).**
> Read `sprint-1-hub/60_REQUIREMENTS_v2.md` §3–§5, the briefs of the phases that were not completed — `sprint-1-hub/briefs/63_GRADING_VALIDATION.md` (V-1, stubbed), `66_SESSION_ARCHIVAL_RAG.md` (V-2, lives in `~/agentic-harness`), `sprint-2/parked/81_PHASE13_styling.md` (Phase 13, skipped, carry-ins C-1..C-3), `sprint-2/82_PHASE14_containers.md` + its six `research/82_*` files (Phase 14, planned) — plus the 12b brief's deferred rows (`80c` §Item task list: P-data-1, P-db-3), the intake's S2-carry items, and STATUS §Known issues. Write `docs/planning/sprint-2/91_REQUIREMENTS_v3.md`: §1 **Carried requirements**, one R-number each continuing from R-29, each with its source, its state today (not built / partly built, with what exists / built but unproven), what is still missing, and the sprint 1 decision it must respect; §2 **PM-added requirements** — intermediate steps you find missing (a carried feature that cannot ship without a seam, a data fix, a test harness), each marked *PM-added* with one line of why and, where you had to look something up, the research note that backs it; §3 **Stack's new features and requirements** — empty, with a template per entry (area · what · why · must/should · acceptance in my words · anything it must not do); §4 **Still declined**, carried from v2 §5 so nothing sneaks back. Do not write phases or prompts yet. STOP and tell me the file is ready for §3; I will fill it in the worktree file or paste the list here for you to transcribe verbatim.
>
> **Stage B — research, on my word "research".**
> Group §1–§3 into conceptual areas (expect 6–12). Spawn one Sonnet researcher per area in parallel, each writing `docs/planning/sprint-2/research/92_RESEARCH_sprint2_<area>.md`: standard practice for that kind of feature, two or three open-source examples with links and what to borrow, known pitfalls, how it maps onto this stack (Next.js 16 + Supabase + Electron + the crawler + pg_cron), a size (S/M/L) and the seams it touches. Then write `93_SPRINT2_RESEARCH_SYNTHESIS.md` yourself: what the research changes about each requirement, requirements it adds (append them to §2 of 91_, marked *research-added*), and ONE batch of questions for me with a default answer each. STOP for my answers.
>
> **Stage C — phases with deterministic tasks, on my answers.**
> Write `94_SPRINT2_PHASES.md`: the phase list and dependency graph, which phases can run in parallel, the migration range per phase (from 091, with slack), and for each phase one brief under `docs/planning/sprint-2/briefs/9N_PHASE<n>_<name>.md` in the sprint 1 shape (`80c` is the template): why, frozen Contract (routes, RPC signatures, tables, file names, seams), MVP in my words, DoD, and an **explicit task list where every task has a deterministic check** — a named test file that must be green, a SQL assertion with its expected value, a screenshot step with what must be visible, a count — nothing that needs judgement to tick. Size phases so one PR is one phase and development can run long: a phase may hold 10–30 tasks. Cite R-numbers everywhere. STOP for my approval of the phase plan.
>
> **Stage D — session prompts and the orchestrator refresh, on my approval.**
> Write one copy-paste prompt per *session* (a session may run several phases at once with parallel Opus subagents; say which phases share a session and why), and refresh `project-state/ORCHESTRATOR.md` for sprint 2: §1 becomes a short standing paragraph plus a sprint 2 phase table (sprint 1's table collapses to a pointer at STATUS; fix the clipped materials-MCP sentence), §2 gets the sprint 2 graph and rules (keep only the term calendar from sprint 1), §3 gains the two sprint 1 lessons "walk direct loads, not only click-throughs" and "a skill step written as prose gets skipped — script it", §4 lists only my standing items, §5 points at the README and the current intake, §6 holds the sprint 2 prompts and moves the used sprint 1 prompts into `sprint-1-hub/` as history. Update STATUS "What's next — Sprint 2" and add DECISIONS rows for every product call I made in stages B and C. Open one docs PR with everything from stages A–D and STOP at "ready when you say so".
>
> Rules that override habit: never commit to `main`; anything I decide goes in DECISIONS with the why; workers get worktrees and their own branches; migrations are additive, applied under the file's name, byte-identical; visual work gets a preview before merge; if another session is active on the repo, fetch first and treat its worktrees as foreign.
