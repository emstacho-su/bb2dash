-- bb2dash :: 075_shared_column_restamp.sql
-- Phase 12b, item X-3 (docs/planning/80c_PHASE12B_page_pass.md: P-data-2). Worker W-30.
--
-- THE BUG. IST.323's gradebook column _3569973_1 is bound to TWO assignments -
-- `IST.323/fp-proposal` (13 pts) and `IST.323/fp-log-final` (2 pts) - because Blackboard collects
-- the whole final-project packet under one column. stage_assignments sees more than one match,
-- correctly refuses to guess which assignment Blackboard's due date and points belong to, raises
-- the "attached to more than one assignment" Inbox row - and then `continue`s out of the loop
-- before the step that mirrors Blackboard's own bookkeeping. So `bb_last_seen` on both rows has
-- sat at 2026-09-02 through every crawl since. Migration 065's rule reads that as
-- `absent_from_blackboard = true`, and push.ts:264 drops every absent item
-- (`desired.filter((item) => !item.absent_from_blackboard)`), so neither of Stack's final-project
-- deadlines has ever reached the Google calendar.
--
-- THE FIX, in two parts.
--   1. The code path: in the ambiguous branch, restamp the LIVENESS fields - `bb_last_seen`, and
--      `bb_item_id` only where it is still null - on every assignment bound to that column before
--      continuing. The ambiguity is about which assignment the column's VALUES describe; it is
--      not about whether Blackboard still has the column. It does, and it reported it in this
--      crawl, so every row bound to it was seen. due_at, points_possible and bb_submission_status
--      are deliberately NOT mirrored here: those are the values Stack still has to arbitrate, and
--      the Inbox row asking him is raised exactly as before.
--   2. The data: one restamp now, so Stack does not have to wait for the next sync. It is written
--      generically (every assignment on a column shared by more than one assignment and present
--      in the newest folded crawl's gradebook payload) and asserted to touch exactly the 2 rows
--      that qualify on prod today.
--
-- The function body below is 045's, verbatim, with that one block added; 045 stays frozen.
-- assignment_progress is not touched: this writes only Blackboard's own bookkeeping columns.
  ...
  begin
    ...
    for c in select * from _bb_cols order by course_id, column_id loop
      ...
      if v_n > 1 then
        ...
        -- and the calendar push filters them out. The values stay untouched until Stack answers
        -- the question raised just above.
        update assignments
...
comment on function stage_assignments(uuid,bigint) is
  ...
  're-asked. A gradebook column bound to more than one assignment still raises the "which one?" '
  'question and still applies none of the column''s values, but since 075 it does restamp '
  'bb_last_seen on every row bound to it, so a shared column no longer looks absent from '
  'Blackboard. assignment_progress is never touched.';
...
-- =============================================================================================
-- The one-off restamp (fix, part 2)
-- =============================================================================================
do $$
declare n int;
begin
  ...
  cols as (
    select cr.id                      as course_id,
           ...
           b.captured_at
      from bb_raw b
      join newest_crawl n on n.run_id = b.run_id
      join courses cr on cr.id = bb_resolve_course(b.bb_course_id),
           lateral jsonb_array_elements(bb_jarray(b.payload->'gradebook')) g
     where b.kind = 'course'
       and coalesce(g->>'columnId','') <> ''
  ...
  shared as (
    select a.id, k.content_id, k.captured_at
      from assignments a
      join cols k on k.course_id = a.course_id and k.column_id = a.bb_column_id
     where (select count(*) from assignments a2
             where a2.course_id = a.course_id and a2.bb_column_id = a.bb_column_id) > 1
  )
  update assignments a
     set bb_item_id   = coalesce(a.bb_item_id, s.content_id),
         bb_last_seen = greatest(coalesce(a.bb_last_seen, s.captured_at), s.captured_at)
    from shared s
   where a.id = s.id
     and (a.bb_item_id   is distinct from coalesce(a.bb_item_id, s.content_id)
       or a.bb_last_seen is distinct from greatest(coalesce(a.bb_last_seen, s.captured_at), s.captured_at));
  get diagnostics n = row_count;
  if n <> 2 then
    raise exception '075: expected to restamp the 2 shared-column assignments, restamped %', n;
  end if;
end $$;
