...
resolution and the `powershell.exe` fallback are as before; `<id>` still matches `^\d{1,12}$` before
...
- [ ] **Stack's acceptance script (on his laptop, from the unpacked build):** (1) double-click
      ...
      opens Windows Terminal in the repo and runs the sync command (C-8 amended). All six ticked.
